...
if ENV['provider_states_url']
  module ProviderStateServerClient
    ...
    def clean_up_state(provider_state)
      puts "Clean up provider state '#{provider_state}' for consumer '#{ENV['pact_consumer']}' using provider state server at #{ENV['provider_states_active_url']}"
      conn = Faraday.new(url: ENV['provider_states_active_url']) do |faraday|
        faraday.adapter Faraday.default_adapter # make requests with Net::HTTP
      end
      conn.delete do |req|
